...
function Layout({ children }: { children: ReactNode }) {
  return <div className="shell"><header><Link to="/workspace" className="brand">Local Agent OS</Link><nav><Link to="/workspace">Workspace</Link><Link to="/skills">Skills</Link><Link to="/approvals">Approvals</Link><Link to="/settings/models">Models</Link></nav></header>{children}</div>;
}
...
function TaskDetail() {
  ...
  return <Layout><main className="grid"><section><Link to="/workspace">← Workspace</Link><h1>{task?.title ?? 'Đang tải...'}</h1><p data-testid="task-state" className="pill">{task?.state}</p><p>{task?.resultSummary || task?.errorMessage || task?.userInput}</p>{task?.state==='waiting_for_approval'&&<p><Link to="/approvals">Review skill proposal to continue this task →</Link></p>}<button className="secondary" disabled={!task || ['completed', 'failed', 'cancelled'].includes(task.state)} onClick={() => void api.cancel(taskId)}>Cancel task</button></section><Timeline/></main></Layout>;
}
...
function Skills(){const{data=[]}=useQuery({queryKey:['skills'],queryFn:api.skills});return <Layout><main><h1>Skill Studio</h1><div className="tasks">{data.map((skill)=><Link key={skill.id} to={`/skills/${skill.id}`}><span><b>{skill.name}</b><br/><small>{skill.manifest.runtime.type} · v{skill.version}</small></span><span className="pill">{skill.status}</span></Link>)}</div></main></Layout>}
function SkillDetail(){const{skillId=''}=useParams();const{data}=useQuery({queryKey:['skill',skillId],queryFn:()=>api.skill(skillId)});const{data:versions=[]}=useQuery({queryKey:['versions',skillId],queryFn:()=>api.versions(skillId)});return <Layout><main><Link to="/skills">← Skill Studio</Link><h1>{data?.name}</h1><div className="card"><p>{data?.description}</p><p>Success rate: {Math.round((data?.successRate??0)*100)}% · Uses: {data?.usageCount??0}</p><h2>Permissions</h2><pre>{JSON.stringify(data?.manifest?.permissions,null,2)}</pre><button className="secondary" onClick={()=>void api.disable(skillId)}>Disable</button></div><h2>Versions</h2><div className="tasks">{versions.map(v=><div className="card" key={v.version}>v{v.version} · {v.createdAt}</div>)}</div></main></Layout>}
function Approvals(){const{data=[],refetch}=useQuery({queryKey:['approvals'],queryFn:api.approvals});return <Layout><main><h1>Approval Center</h1><div className="tasks">{data.map(a=><div className="card" key={a.id}><h2>{a.proposal.name}</h2><p>{a.proposal.reason}</p><p><span className="pill">{a.status}</span> · {a.proposal.runtimeType} · risk {a.proposal.riskLevel}</p><pre>{JSON.stringify(a.proposal.permissions,null,2)}</pre>{a.status==='pending'&&<div className="actions"><button onClick={()=>void api.approve(a.id).then(()=>refetch())}>Approve version</button><button className="secondary" onClick={()=>void api.reject(a.id).then(()=>refetch())}>Reject</button></div>}</div>)}</div></main></Layout>}

export function App() {
  return <Routes><Route path="/workspace" element={<Workspace/>}/><Route path="/tasks/:taskId" element={<TaskDetail/>}/><Route path="/skills" element={<Skills/>}/><Route path="/skills/:skillId" element={<SkillDetail/>}/><Route path="/approvals" element={<Approvals/>}/><Route path="/settings/models" element={<Models/>}/><Route path="*" element={<Workspace/>}/></Routes>;
}
